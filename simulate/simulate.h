...
class Simulate {
   public:
    ...
    Simulate(std::unique_ptr<PlatformUIAdapter> platform_ui_adapter,
             mjvScene* scn, mjvCamera* cam, mjvOption* opt, mjvPerturb* pert,
             bool fully_managed, void* visualize_func_p = nullptr);
    ...
    // whether the viewer is operating in fully managed mode, where it can
    // assume that it has exclusive access to mjModel, mjData, and various mjv
    // objects
    ...
    // mjModel and mjData fields that can be modified by the user through the
    // GUI
    ...
    std::atomic_int uiloadrequest = 1;

...
void MakePhysicsSection(mj::Simulate *sim, int oldstate) {
  mjOption *opt =
      sim->fully_managed_ ? &sim->m_->opt : &sim->scnstate_.model.opt;
  mjuiDef defPhysics[] = {
...
void MakeVisualizationSection(mj::Simulate *sim, const mjModel *m,
                              int oldstate) {
  mjStatistic *stat =
      sim->fully_managed_ ? &sim->m_->stat : &sim->scnstate_.model.stat;
  mjVisual *vis =
      sim->fully_managed_ ? &sim->m_->vis : &sim->scnstate_.model.vis;
...
void MakeControlSection(mj::Simulate *sim, int oldstate) {
  ...
  for (int i = 0; i < sim->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM;
    ...
    if (!sim->opt.actuatorgroup[mjMAX(
            0, mjMIN(mjNGROUP - 1, sim->actuator_group_[i]))]) {
      continue;
    }
  ...
  if (state->type == mjEVENT_KEY && state->key != 0) {
    ...
    case ']': // cycle up fixed cameras
      if ((sim->m_ || !sim->fully_managed_) && sim->ncam_) {
        sim->cam.type = mjCAMERA_FIXED;
        // camera = {0 or 1} are reserved for the free and tracking
        // cameras
...
Simulate::Simulate(std::unique_ptr<PlatformUIAdapter> platform_ui,
                   mjvScene *scn, mjvCamera *cam, mjvOption *opt,
                   mjvPerturb *pert, bool fully_managed, void *visualize_func_p)
    : fully_managed_(fully_managed), scn(*scn), cam(*cam), opt(*opt),
      pert(*pert), platform_ui(std::move(platform_ui)),
      uistate(this->platform_ui->state()), ModifySceneFunc(visualize_func_p) {
...
#undef X
  ...
  if (fully_managed_) {
    mjv_updateScene(m_, d_, &this->opt, &this->pert, &this->cam, mjCAT_ALL,
                    &this->scn);
    if (ModifySceneFunc) {
      ((void (*)(mjvScene *))ModifySceneFunc)(&this->scn);
    }
  } else {
  ...
  if (this->run || !this->fully_managed_) {
    ...
    mjv_applyPerturbPose(m_, d_, &this->pert,
                         1); // mocap and dynamic bodies
  }
}
...
void Simulate::Render() {
  ...
  if (this->ui0_enable && this->ui0.sect[SECT_WATCH].state) {
    mjui_update(SECT_WATCH, -1, &this->ui0, &this->uistate,
                &this->platform_ui->mjr_context());
  }

  if (pending_.ui_update_physics) {
    if (this->ui0_enable && this->ui0.sect[SECT_PHYSICS].state) {
      mjui_update(SECT_PHYSICS, -1, &this->ui0, &this->uistate,
                  &this->platform_ui->mjr_context());
    }
  ...
  if (!pauseloadlabel.empty() || rtlabel[0]) {
    ...
    mjr_overlay(mjFONT_BIG, mjGRID_TOPLEFT, smallrect, topleftlabel.c_str(),
  ...
  if (this->screenshotrequest.exchange(false)) {
    ...
    // filename and append .PNG, the macOS file dialog does not
    // automatically open that location. Thus, we defer to a default
